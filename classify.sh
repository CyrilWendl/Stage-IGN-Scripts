...
for methode in "S2" "SPOT6";do
	...
	$DIR_EXES/Legende label2RVB ../legende.txt classif_$methode.rle classif_$methode.visu.tif	
...
for DNAME in "$DIR_SAVE/Fusion_all" "$DIR_SAVE/Fusion_all_weighted";do
 # target directory 
	...
	for i in $DNAME/*.tif ; do
		...
		echo -n "$DIR_EXES/Legende label2RVB $DNAME/../legende.txt $DNAME/Classified/classif_Fusion_$methode.rle $DNAME/Classified/classif_Fusion_$methode.visu.tif; ">> bashtmp.sh
